...
import com.cgy.weixin.api.redpacket.SignService;
import com.cgy.weixin.app.base.BaseApp;
...
import com.cgy.weixin.model.message.DeleteContactMessage;
...
public class MyApp extends BaseApp implements RongIMClient.OnReceiveMessageListener{
    ...
    public void onCreate() {
        ...
        initImagePicker();
        //初始化表情控件
        LQREmotionKit.init(this, (Context context, String path, ImageView imageView) -> {
            Glide.with(context).load(path).centerCrop().diskCacheStrategy(DiskCacheStrategy.SOURCE)
                    .into(imageView);
        });
        //初始化ShareSDK
    }
...
//                        BroadcastManager.getInstance(getContext()).sendBroadcast(AppConst.UPDATE_CONVERSATIONS)
    ...
    private void handleGroupDismiss(String groupId) {
                ...
                call.enqueue(new Callback<SignModel>() {
                    ...
                    public void onResponse(Call<SignModel> call, Response<SignModel> response) {
                        if (response.isSuccessful()) {
                            ...
                            rpValueCallback.onError(statusCode, response.errorBody().toString());
                    ...
                    public void onFailure(Call<SignModel> call, Throwable t) {
                        ...
                        rpValueCallback.onError("onFailure", t.getMessage());
                    }
                });
