...
public class VideoThumbLoader {
    ...
    public void showThumb(String path, ImageView imgView, int width, int height) {
        if (getVideoThumbToCache(path) == null) {
            //异步加载
            imgView.setTag(path);
            new MyBobAsyncTask(imgView, path, width, height).execute(path);
        } else {
    ...
    class MyBobAsyncTask extends AsyncTask<String, Void, Bitmap> {
        ...
        public MyBobAsyncTask(ImageView imageView, String path, int width, int height) {
    ...
    private void showDateByThread(ImageView imageView, final String path, final int width, final int height) {
        imgView = imageView;
        this.path = path;
        new Thread(new Runnable() {
            @Override
            public void run() {
                Bitmap bitmap = createVideoThumbnail(path, width, height, MediaStore.Video.Thumbnails.MICRO_KIND);
    ...
    private static Bitmap createVideoThumbnail(String videoPath, int width, int height, int kind) {
