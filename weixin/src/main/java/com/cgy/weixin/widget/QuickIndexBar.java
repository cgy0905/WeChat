...
public class QuickIndexBar extends View{
    ...
    public QuickIndexBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//设置开启抗锯齿
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < LETTERS.length; i++) {
            ...
            //计算坐标
            //x = 格子宽度的一半 - 文字宽度的一半
            //int x = (int) (getMeasuredWidth() / 2.0f);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                index = (int) (event.getY() / mCellHeight); //y / 每个单元格的高度 = 当前单元格的索引
