...
import com.cgy.weixin.utils.ImageUtils;
 ...
 * 会话界面 (单聊、群聊)
 */
public class SessionActivity extends BaseFragmentActivity<ISessionView, SessionPresenter> implements ISessionView, IEmotionSelectedListener, BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    public void init() {
        ...
        int sessionType = getIntent().getIntExtra("sessionType", SESSION_TYPE_PRIVATE);
        switch (sessionType) {
    ...
    public void initListener() {
        ...
        mRlTakePhoto.setOnClickListener(v -> {
            Intent intent = new Intent(SessionActivity.this, TakePhoneActivity.class);
        ...
        AudioRecordManager.getInstance(this).setAudioRecordListener(new IAudioRecordListener() {
            ...
            public void setRecordingTipView() {
                if (this.mRecordWindow != null) {
                    ...
                    this.mStateTV.setVisibility(View.VISIBLE);
                    this.mTimerTV.setText(R.string.voice_rec);
                    this.mTimerTV.setBackgroundResource(R.drawable.bg_voice_popup);
    ...
    private boolean isCanceled(View view, MotionEvent event) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case REQUEST_IMAGE_PICKER:
                if (resultCode == ImagePicker.RESULT_CODE_ITEMS) {//返回多张图片
                    if (data != null) {
                        //是否发送原图
                        boolean isOrig = data.getBooleanExtra(ImagePreviewActivity.ISORIGIN, false);
                        ArrayList<ImageItem> images = (ArrayList<ImageItem>) data.getSerializableExtra(ImagePicker.EXTRA_RESULT_ITEMS);
                        Log.e("CGY", isOrig ? "发原图" : "不发原图");//若不发原图的话,需要自己在项目中做好压缩图片算法
            ...
            case REQUEST_MY_LOCATION:
                if (requestCode == RESULT_OK) {
                    ...
                    mPresenter.sendLocationMsg(locationData);
                }
                break;
        }
    ...
    private void registerBR() {
        ...
        RongIMClient.setTypingStatusListener((type, targetId, typingStatusSet) -> {
            ...
            if (type.equals(mConversationType) && targetId.equals(mSessionId)) {
                //count表示当前会话中正在输入的用户数量,目前只支持单聊,所欲判断大于0就可以给予显示
                ...
                if (count > 0) {
                    ...
                    MessageTag voiceTag = TextMessage.class.getAnnotation(MessageTag.class);
                    //匹配对方正在输入的文本消息还是语音消息
        ...
        //监听撤回
        RongIMClient.getInstance().setRecallMessageListener((messageId, recallNotificationMessage) -> {
    ...
    protected SessionPresenter createPresenter() {
        return new SessionPresenter(this, mSessionId, mConversationType);
    }
