...
import com.cgy.weixin.manager.JsonManager;
import com.cgy.weixin.model.cache.UserCache;
import com.cgy.weixin.model.data.GroupNotificationMessageData;
...
import com.cgy.weixin.utils.TimeUtils;
import com.cgy.weixin.utils.UIUtils;
import com.cgy.weixin.utils.VideoThumbLoader;
import com.cgy.weixin.widget.BubbleImageView;
import com.cgy.weixin.widget.CircularProgressBar;
...
public class SessionAdapter extends LQRAdapterForRecyclerView<Message>{
    ...
    public SessionAdapter(Context context, List<Message> data, SessionPresenter presenter) {
        super(context, data);
        mContext = context;
        mData = data;
    ...
    private void setView(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (msgContent instanceof TextMessage) {
            ...
            Glide.with(mContext).load(imageMessage.getLocalUri() == null ?
            imageMessage.getRemoteUri() : imageMessage.getLocalUri())
                    .error(R.mipmap.default_img_failed)
                    .override(UIUtils.dip2Px(80), UIUtils.dip2Px(150))
                    .centerCrop().into(bivPic);
            ...
            if (MediaFileUtils.isImageFileType(fileMessage.getName())) {
                ...
                Glide.with(mContext).load(fileMessage.getLocalPath() == null ?
                fileMessage.getMediaUrl() : fileMessage.getLocalPath())
                ...
                if (fileMessage.getLocalPath() != null && new File(fileMessage.getLocalPath().getPath()).exists()) {
                    VideoThumbLoader.getInstance().showThumb(fileMessage.getLocalPath().getPath(),
                            bivPic, 200, 200);
                } else {
            ...
            Glide.with(mContext).load(locationMessage.getImgUri())
                    .placeholder(R.mipmap.default_location)
                    .centerCrop()
                    .into(ivLocation);
            ...
            try {
                ...
                GroupNotificationMessageData data = JsonManager.jsonToBean(groupNotificationMessage.getData(), GroupNotificationMessageData.class);
    ...
    private void setStatus(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (msgContent instanceof TextMessage || msgContent instanceof  LocationMessage || msgContent instanceof  VoiceMessage) {
            ...
            if (MediaFileUtils.isImageFileType(fileMessage.getName())) {
                ...
                if (isSend) {
                    Message.SentStatus sentStatus = item.getSentStatus();
                    if (sentStatus == Message.SentStatus.SENDING || fileMessage.getLocalPath()
                            == null || (fileMessage.getLocalPath() != null
                            && !new File(fileMessage.getLocalPath().getPath()).exists())) {
                        if (!TextUtils.isEmpty(item.getExtra())) {
    ...
    private void setTime(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (position > 0) {
            ...
            if (msgTime - preMsgTime > (5 * 60 * 1000)) {
                helper.setViewVisibility(R.id.tvName, View.VISIBLE).setText(R.id.tvName, TimeUtils.getMsgFormatTime(msgTime));
            } else {
