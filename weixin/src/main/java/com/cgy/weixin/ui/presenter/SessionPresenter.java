...
import com.cgy.weixin.model.message.RedPacketMessage;
import com.cgy.weixin.ui.activity.SessionActivity;
import com.cgy.weixin.ui.activity.ShowBigImageActivity;
import com.cgy.weixin.ui.adapter.SessionAdapter;
import com.cgy.weixin.ui.base.BaseFragmentActivity;
import com.cgy.weixin.ui.base.BaseFragmentPresenter;
import com.cgy.weixin.ui.view.ISessionView;
import com.cgy.weixin.utils.FileOpenUtils;
import com.cgy.weixin.utils.LogUtils;
import com.cgy.weixin.utils.MediaFileUtils;
 ...
 * 2018/6/5  10:15
 */
public class SessionPresenter extends BaseFragmentPresenter<ISessionView> {
    ...
    public SessionPresenter(BaseFragmentActivity context, String sessionId, Conversation.ConversationType conversationType) {
        super(context);
        ...
        if (mAdapter == null) {
            ...
            mAdapter.setOnItemClickListener((helper, parent, itemView, position) -> {
                ...
                if (content instanceof ImageMessage) {
                    ...
                    String receiveId = UserCache.getId();
                    String direct = RPConstant.MESSAGE_DIRECT_RECEIVE;
                    RedPacketUtil.openRedPacket((SessionActivity) mContext, chatType, redPacketId, redPacketType, receiveId, direct);
                }
            });
    ...
    private void downloadMediaMessage(Message message) {
    ...
    public void resetDraft() {
        rx.Observable.just(RongIMClient.getInstance().getTextMessageDraft(mConversationType, mSessionId))
    ...
    private void getRemoteHistoryMessages() {
        //消息中的 sentTime 第一次可传 0 获取最新的 count条
        long dateTime = 0;
        if (mData.size() > 0) {
    ...
    public void sendLocationMsg(LocationData locationData) {
        LocationMessage message = LocationMessage.obtain(locationData.getLat(),
                locationData.getLng(), locationData.getPoi(), Uri.parse(locationData.getImgUrl()));
    ...
    RPSendPacketCallback RPSendPacketCallback = new RPSendPacketCallback() {
        ...
        public void onSendPacketSuccess(RedPacketInfo redPacketInfo) {
            RedPacketMessage rpMsg = RedPacketMessage.obtain(redPacketInfo.redPacketId,
                    redPacketInfo.fromNickName, redPacketInfo.redPacketType, redPacketInfo.redPacketGreeting);
            RongIMClient.getInstance().sendMessage(Message.obtain(mSessionId, mConversationType, rpMsg),
                    mPushContent, mPushData, new IRongCallback.ISendMessageCallback() {
                        @Override
                        public void onAttached(Message message) {
                            //保存数据库成功
                            mAdapter.addLastItem(message);
